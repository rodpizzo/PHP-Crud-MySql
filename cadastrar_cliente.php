...
$sucess_msg='';
    ...
    if (!empty($telefone) && !preg_match('/^\d+$/', $telefone)) {
        ...
        $sql_code = "INSERT INTO clientes(nome, email, telefone, nascimento, data) VALUES('$nome', '$email', '$telefone', '$dataNascimento', NOW())";
        $deucerto = $mysqli->query($sql_code) or die($mysqli->error);
        if ($deucerto) {
            $sucess_msg= "<p>Cliente Cadastrado com Sucesso!<p>";
            unset($_POST);
        }
    }
}
...
<body>
    <div class="login">
        ...
        <form method="post">
            ...
            <input type="tel" name="Telefone" placeholder="Telefone apenas números" />
